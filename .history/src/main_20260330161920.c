		...
		for (int y = draw_start; y < draw_end; y++)
{
		...
		//int color = ((int *)data->tex_test.addr)[tex_y * TEX_W + tex_x];
		int color = *(int *)(data->tex_test.addr
            + tex_y * data->tex_test.line_len
            + tex_x * (data->tex_test.bpp / 8));
...
void load_textures(t_data *data)
{
    ...
    data->tex_test[0].img_ptr = mlx_xpm_file_to_image(data->mlx, "textures/north.xpm", &w, &h);
    data->tex_test[1].img_ptr = mlx_xpm_file_to_image(data->mlx, "textures/south.xpm", &w, &h);
    data->tex_test[2].img_ptr = mlx_xpm_file_to_image(data->mlx, "textures/west.xpm",  &w, &h);
    data->tex_test[3].img_ptr = mlx_xpm_file_to_image(data->mlx, "textures/east.xpm",  &w, &h);
    ...
    while (i < 4)
    {
        ...
        {
            write(2, "Erro: textura nao encontrada\n", 29);
            exit(1);
        }
        data->tex_test[i].addr = (int *)mlx_get_data_addr(data->tex_test[i].img_ptr,
                                &data->tex_test[i].bpp,

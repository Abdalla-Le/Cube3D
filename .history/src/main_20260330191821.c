...
{
    int tile = 8;      // tamanho de cada célula em pixels
    int offset_x = 10; // posição do minimapa na tela (canto superior esquerdo)
    int offset_y = 10;
    int i;
    int j;
    int color;
    ...
    while (data->real_map.grid[i] != NULL)
    {
        ...
        while (data->real_map.grid[i][j] != -1)
        {
            if (data->real_map.grid[i][j] == 1)
                ...
                color = 0x000000; // vazio = preto

            // Desenha o tile (quadradinho)
    ...
    // Desenha o player como um quadradinho colorido
    int px = offset_x + (int)(data->player.pos_y * tile);
    int py = offset_y + (int)(data->player.pos_x * tile);
    for (int dy = -2; dy <= 2; dy++)
        for (int dx = -2; dx <= 2; dx++)
            my_mlx_pixel_put(data, px + dx, py + dy, 0xFF0000); // vermelho
}

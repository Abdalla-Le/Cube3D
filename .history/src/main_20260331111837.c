...
void    draw_minimap(t_data *data)
{
    ...
    for (int dy = 0; dy < map_h * tile; dy++)
        for (int dx = 0; dx < map_w * tile; dx++)
            my_mlx_pixel_put(data, offset_x + dx, offset_y + dy, 0x111111);
    ...
    while (data->real_map.grid[i] != NULL)
    {
        ...
        while (data->real_map.grid[i][j] != -1)
        {
            ...
            for (int dy = 1; dy < tile - 1; dy++)   // -1 pra deixar borda
                for (int dx = 1; dx < tile - 1; dx++)
                    my_mlx_pixel_put(data,
                        offset_x + j * tile + dx,  // j = coluna = Y da tela
                        offset_y + i * tile + dy,  // i = linha  = X da tela
                        color);
            j++;
        }
        i++;
    ...
    // Player — pos_y é coluna, pos_x é linha
...
int **convert_grid(char **char_grid)
{
    int i;
    int j;
    int **int_grid;
    ...
    while (char_grid[i] != NULL)
    {
        ...
        while (char_grid[i][j] != '\0')
        {
            if (char_grid[i][j] == '1')
                ...
                int_grid[i][j] = -1; // espaço vazio fora do mapa
            else
                int_grid[i][j] = 0; // N/S/E/W viram 0 (find_player trata depois)
            j++;
        }
        int_grid[i][j] = -1; // sentinela de fim de linha
        i++;
    }
    int_grid[i] = NULL;
    return (int_grid);
...
int	main(int ac, char **av)
{
	...
	data.real_map = *map;
	data.map = convert_grid(map->grid);
	find_player(&data);
	load_texture(&data);
	//init_player(&data);

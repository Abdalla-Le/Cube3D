...
/*   Updated: 2026/03/31 13:36:10 by eteofilo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	raycasting_loop(t_data *data)
{
	...
	while (x < W_WIDTH)
	{
		...
		ray_dir[0] = data->player.dir_x + data->player.plane_x * camera_x;
		ray_dir[1] = data->player.dir_y + data->player.plane_y * camera_x;

		map[0] = (int)data->player.pos_x;
		map[1] = (int)data->player.pos_y;

		delta_dist[0] = fabs(1 / ray_dir[0]); //variação da distância do raio; TL,DR: Quanto custa para um raio atravessar um bloco;
		...
		{
			step[0] = -1;
			side_dist[0] = (data->player.pos_x - map[0]) * delta_dist[0]; // pos_x - map[0] = posição real dentro do bloco; * delta_dist = distância real até o limite do bloco
		}
		else
		{
			step[0] = 1;
			side_dist[0] = (map[0] + 1.0 - data->player.pos_x) * delta_dist[0];
		}
		if (ray_dir[1] < 0)
		{
			step[1] = -1;
			side_dist[1] = (data->player.pos_y - map[1]) * delta_dist[1];
		}
		else
		{
			step[1] = 1;
			side_dist[1] = (map[1] + 1.0 - data->player.pos_y) * delta_dist[1];
		}
		...
		while (hit == 0)
		{
			...
			if (data->map[map[0]][map[1]] > 0)
				hit = 1;
		}

		// --- CÁLCULO FINAL ---
		if (side == 0)
			perp_wall_dist = (side_dist[0] - delta_dist[0]); //subtrai um passo
		else
		...
		int tex_index;
		if (side == 0)
			tex_index = (ray_dir[0] > 0) ? 2 : 3; // WEST ou EAST
		else
			tex_index = (ray_dir[1] > 0) ? 0 : 1; // NORTH ou SOUTH
		...
		double wall_x;
		if (side == 0)
			wall_x = data->player.pos_y + perp_wall_dist * ray_dir[1];
		else
			wall_x = data->player.pos_x + perp_wall_dist * ray_dir[0];
		wall_x -= floor(wall_x);

...
/*   Updated: 2026/03/31 14:19:53 by eteofilo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (raycast->side == 0)
		raycast->perp_wall_dist = (raycast->side_dist[0] - raycast->delta_dist[0]);
	else
		raycast->perp_wall_dist = (raycast->side_dist[1] - raycast->delta_dist[1]);
...
void paint_textures(t_img cur_tex, t_raycast *raycast, t_data *data)
{
	double	wall_x;

		if (raycast->side == 0)
			wall_x = data->player.pos_y + raycast->perp_wall_dist * raycast->ray_dir[1];
		else
			wall_x = data->player.pos_x + raycast->perp_wall_dist * raycast->ray_dir[0];
		wall_x -= floor(wall_x);

		int tex_x = (int)(wall_x * (double)TEX_W);
		if ((raycast->side == 0 && raycast->ray_dir[0] > 0) || (raycast->side == 1 && raycast->ray_dir[1] < 0))
			tex_x = TEX_W - tex_x - 1;

		for (int y = raycast->draw_start; y < raycast->draw_end; y++)
		{
			int d     = y * 256 - W_HEIGHT * 128 + raycast->line_height * 128;
			int tex_y = ((d * TEX_H) / raycast->line_height) / 256;
...
t_img	*set_textures(t_raycast *raycast, t_data *data)
{
	int tex_index;
	t_img cur_tex;
	...
	paint_textures(data->tex_test[tex_index], &raycast, data);
}
...
int	raycasting_loop(t_data *data)
{
	...
	while (x < W_WIDTH)
	{
		...
		set_walls(&raycast);
		cur_tex = set_textures(&raycast, data);
		x++;
	}

...
/*   Updated: 2026/03/31 16:20:34 by eteofilo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_dda(t_raycast *raycast, t_data *data)
{
	...
	while (hit == 0)
	{
		...
		if (raycast->map[0] < 0 || raycast->map[1] < 0
			|| raycast->map[0] >= data->map_size[1]
			|| raycast->map[1] >= data->map_size[0]
			|| data->map[raycast->map[0]][raycast->map[1]] > 0)
			hit = 1;
	}
}
